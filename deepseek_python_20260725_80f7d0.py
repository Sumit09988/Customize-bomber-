...
║  PROTECTED NUMBERS · ALERT ON BOMBING ATTEMPT                 ║
╚══════════════════════════════════════════════════════════════╝
...
class UserSession:
    """Per-user session to track active sends independently."""
...
# ═══════════════════════════════════════════════════════════════
# NEW: Protected Numbers Functions
# ═══════════════════════════════════════════════════════════════

def is_number_protected(number: str, d: dict) -> bool:
    """Check if a number is in the protected list."""
    protected = d.get("protected_numbers", [])
    # Normalize number: remove spaces and plus sign for comparison
...
async def notify_protected_attempt(bot: Bot, uid: int, number: str, d: dict):
    """Notify owners and admins about a protected number bomb attempt."""
    ...
    notify_ids = set()
    notify_ids.update(d.get("owners", []))
    notify_ids.update(d.get("admins", []))
    notify_ids.add(MAIN_OWNER)
    
    for nid in notify_ids:
        try:
            await bot.send_message(nid, alert_msg, parse_mode="HTML", disable_web_page_preview=True)
        except Exception as e:
            log.warning(f"Failed to notify {nid}: {e}")

# ═══════════════════════════════════════════════════════════════
# Background scanner with 1-minute interval
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# NEW: Protected Numbers Menu Functions
# ═══════════════════════════════════════════════════════════════
...
def protected_list_text(d: dict) -> str:
    protected = d.get("protected_numbers", [])
    if not protected:
...
async def user_got_number(msg: Message, state: FSMContext):
    ...
    # NEW: Check if number is protected
    d = load()
    if is_number_protected(number, d):
        ...
        await msg.answer(
            ...
            f"⚠️ You cannot send SMS to this number. This attempt has been reported to the admin.",
...
async def user_got_count(msg: Message, state: FSMContext):
    ...
    if is_number_protected(number, d):
        ...
        await msg.answer(
            f"🚫 <b>This number is PROTECTED!</b>\n\n"
            f"📞 Number: <code>{number}</code>\n\n"
            f"⚠️ This attempt has been reported to the admin.",
            reply_markup=kb([("🏠 Home", "user:home")]),
            parse_mode="HTML"
        )
        return
...
# ═══════════════════════════════════════════════════════════════
# run_sms_blast_with_progress - ULTRA EDITION
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# NEW: Protected Numbers Handlers
# ═══════════════════════════════════════════════════════════════
...
async def protected_remove_do(cq: CallbackQuery, state: FSMContext):
    ...
    if number in protected:
        ...
        save(d)
        await cq.answer(f"🗑 {number} removed!", show_alert=True)
    else:
