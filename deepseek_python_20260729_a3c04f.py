# ARYAN_PATCHED_V19 - ULTRA OPTIMIZED (FIXED)
"""
╔══════════════════════════════════════════════════════════════╗
║           SMS BLAST BOT  v3.1 — ULTRA EDITION               ║
║  Per-User Sessions · Concurrent · 1000+ Users Smooth        ║
...
║  ═════════════════════════════════════════════════════════   ║
║  NEW FEATURES:                                              ║
║  1. Number Protection (Only Owner/Super Admin)              ║
║  2. Add Credits to All Users with Notification              ║
║  3. Number Tracker with Username/UserID                    ║
║  4. Credit Transfer (Half of total) for Normal Users       ║
║  aiogram 3.x · aiohttp · JSON Storage                       ║
║  FIXED: Duplicate startup prevention                       ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# ========== GLOBAL STARTUP GUARDS ==========
# Prevent duplicate startup across Railway restarts
_BOT_STARTED = False
_SCANNER_STARTED = False
_STARTUP_NOTIFIED = False
_SCANNER_ACTIVE_ID = None

# ========== CONFIG ==========
MAIN_OWNER = 7515864015  # Updated Owner ID
SUPER_ADMIN_NAME = "@T4HKR"  # Updated Username
SUPER_ADMIN_LINK = "tg://user?id=" + str(MAIN_OWNER)

SUPER_ADMINS = [7515864015]  # Updated

BOT_TOKEN = "8463766338:AAFrcI21K7QaID2OplD43qg41IY4JcsnsI4"  # Updated Token
...
PROTECTED_NUMBERS = {}  # {number: protector_uid}
...
def mask_number(number: str) -> str:
    """Mask number for non-owner users"""
    if len(number) <= 4:
        return number
...
async def background_firebase_scanner(bot: Bot):
    ...
    # Prevent duplicate scanner instance with instance tracking
    scanner_id = f"{time.time()}_{id(bot)}"
    if globals().get('_SCANNER_ACTIVE_ID', None) is not None:
        log.warning("⚠️ Another scanner instance is already running, skipping...")
        return
    
    globals()['_SCANNER_ACTIVE_ID'] = scanner_id
    log.info(f"🔄 Background Firebase Scanner STARTED (ID: {scanner_id[:8]}) — scans every 1 minute")
    first_scan_done = False

    while True:
        try:
            # Check if this scanner should still be active
            if globals().get('_SCANNER_ACTIVE_ID') != scanner_id:
                log.info(f"⏹️ Scanner {scanner_id[:8]} replaced by newer instance, stopping...")
                break
            ...
            log.error(f"[BG-SCAN] Loop error: {e}")
            await asyncio.sleep(5)
...
async def admin_got_number(msg: Message, state: FSMContext):
    ...
    # Admins also cannot send to protected numbers
    if number in PROTECTED_NUMBERS:
        protector_uid = PROTECTED_NUMBERS[number]
        # Allow if admin is the protector or owner
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    ...
    async with SESSIONS_LOCK:
        ...
        if not session or (session.task and session.task.done()):
            ...
            return
        if session.cancelled:
            await cq.answer("⏳ Already stopping...", show_alert=True)
            return
...
async def owner_track_done(msg: Message, state: FSMContext):
    ...
    # Find all users who sent to this number
    users_who_sent = []
    for uid_str, udata in d.get("users", {}).items():
        for entry in udata.get("sms_history", []):
...
# ========== EXISTING HANDLERS (Owner Panel, etc.) ==========
...
def get_scan_status() -> str:
    ...
    global SCAN_STATUS, LAST_SCAN_TIME
    if SCAN_STATUS and "Error" in SCAN_STATUS:
        return f"⚠️ {SCAN_STATUS}"
    if not SCAN_STATUS or SCAN_STATUS == "⏳ Not started":
        return "⏳ Initializing..."
    if LAST_SCAN_TIME:
        age = int(time.time() - LAST_SCAN_TIME)
        return f"{SCAN_STATUS} ({age}s ago)"
    return SCAN_STATUS or "⏳ Scanning..."

async def main():
    # Prevent duplicate startup
    if globals().get('_BOT_STARTED', False):
        log.warning("⚠️ Bot already started, skipping duplicate initialization")
        return
    ...
    # CRITICAL: Delete webhook before polling to prevent duplicate messages
    ...
    # Only create scanner once per process
    if not globals().get('_SCANNER_STARTED', False):
        globals()['_SCANNER_STARTED'] = True
        scanner_task = asyncio.create_task(background_firebase_scanner(bot))
        log.info("🔄 Background scanner task created (first time)")
    else:
        log.info("⏭️ Scanner already running, skipping duplicate creation")

    # Send startup notification only once
    if not globals().get('_STARTUP_NOTIFIED', False):
        globals()['_STARTUP_NOTIFIED'] = True
        try:
            await bot.send_message(
                MAIN_OWNER,
                ...
                f"🛡️ <b>Duplicate Protection:</b> ACTIVE",
                parse_mode="HTML"
            )
        except Exception as e:
...
if __name__ == "__main__":
    # Prevent multiple asyncio runs in Railway environment
    try:
        asyncio.run(main())
    except RuntimeError as e:
        if "cannot be used in a" in str(e).lower() or "event loop" in str(e).lower():
            log.warning(f"⚠️ Asyncio runtime error: {e}")
