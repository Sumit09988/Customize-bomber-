# ARYAN_PATCHED_V19 - ULTRA OPTIMIZED - COMPLETE FIXED VERSION
"""
╔══════════════════════════════════════════════════════════════╗
║           SMS BLAST BOT  v3.1 — ULTRA EDITION               ║
║  Fixed: Data Corruption • Auto-Restart • Backup System      ║
║  Fixed: Session Management • Memory Leak • Crash Recovery   ║
║  Fixed: Token Issue • SSL Error • Event Loop                ║
║  ═════════════════════════════════════════════════════════   ║
║  ALL FIXES APPLIED - STABLE VERSION                         ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# ✅ FIXED TOKEN - YEH USE KARO
BOT_TOKEN = "8463766338:AAETo5sr54aZJ-q1dfYhr9-0DlxZD0Z6lIQ"

# Environment variable support
if not BOT_TOKEN:
    BOT_TOKEN = os.getenv("BOT_TOKEN")
    if not BOT_TOKEN:
        log.error("❌ BOT_TOKEN not found! Please set token.")
        sys.exit(1)

_DATA_FILE = "blast_data.json"
_BACKUP_FILE = _DATA_FILE + ".backup"
_TEMP_FILE = _DATA_FILE + ".tmp"
_HISTORY_DIR = "data_history"
_VERSION = "v3.1-ULTRA-FIXED"
...
PROTECTED_NUMBERS = {}
LAST_GOOD_DATA = None
DATA_SAVE_LOCK = asyncio.Lock()
BOT_RUNNING = True
START_TIME = time.time()
_last_history_backup = 0
...
def validate_data(data: dict) -> bool:
    ...
    required_keys = ['users', 'owners', 'admins', 'banned', 'stats', 'firebases']
...
def save_data(d: dict) -> bool:
    """Atomic save with backup + history"""
    global LAST_GOOD_DATA, _last_history_backup
    
    try:
        LAST_GOOD_DATA = d.copy()
        ...
        # 3. Replace main file
        os.replace(_TEMP_FILE, _DATA_FILE)
        
        # 4. History backup har 10 minute
        now = time.time()
        if now - _last_history_backup >= 600:
            _last_history_backup = now
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            history_file = os.path.join(_HISTORY_DIR, f"blast_data_{timestamp}.json")
            try:
                shutil.copy2(_DATA_FILE, history_file)
                # Sirf last 24 history files rakho
                files = sorted(glob.glob(os.path.join(_HISTORY_DIR, "blast_data_*.json")))
                if len(files) > 24:
                    for f in files[:-24]:
                        try:
                            os.remove(f)
                ...
                log.warning(f"History backup failed: {e}")
        ...
        if os.path.exists(_BACKUP_FILE):
            try:
                shutil.copy2(_BACKUP_FILE, _DATA_FILE)
                log.info("✅ Restored from backup after save failure")
            except Exception as restore_error:
                log.error(f"❌ Backup restore failed: {restore_error}")
        
        return False

def load_data() -> dict:
    """Load data with recovery from backup, history, and memory"""
    global LAST_GOOD_DATA, PROTECTED_NUMBERS
    ...
    if os.path.exists(_DATA_FILE):
        try:
            ...
            if validate_data(data):
                log.info(f"✅ Data loaded from main file. Users: {len(data.get('users', {}))}")
                LAST_GOOD_DATA = data.copy()
                PROTECTED_NUMBERS = data.get("protected_numbers", {})
                return data
            else:
                log.error("❌ Main data file validation failed!")
        except Exception as e:
            log.error(f"❌ Load from main file failed: {e}", exc_info=True)
    ...
    if os.path.exists(_BACKUP_FILE):
        try:
            with open(_BACKUP_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            if validate_data(data):
                log.info("✅ Data restored from backup file!")
                shutil.copy2(_BACKUP_FILE, _DATA_FILE)
                LAST_GOOD_DATA = data.copy()
                PROTECTED_NUMBERS = data.get("protected_numbers", {})
                return data
            else:
                log.error("❌ Backup data validation failed!")
        except Exception as e:
            log.error(f"❌ Load from backup failed: {e}", exc_info=True)
    
    # Try latest history file
    try:
        history_files = sorted(glob.glob(os.path.join(_HISTORY_DIR, "blast_data_*.json")))
        if history_files:
            latest_history = history_files[-1]
            with open(latest_history, "r", encoding="utf-8") as f:
                data = json.load(f)
            if validate_data(data):
                log.info(f"✅ Data restored from history file: {os.path.basename(latest_history)}")
                shutil.copy2(latest_history, _DATA_FILE)
                LAST_GOOD_DATA = data.copy()
                PROTECTED_NUMBERS = data.get("protected_numbers", {})
                return data
    except Exception as e:
        log.error(f"❌ Load from history failed: {e}", exc_info=True)
    
    # Try memory cache
    if LAST_GOOD_DATA:
        log.warning("⚠️ Using cached data from memory!")
        save_data(LAST_GOOD_DATA)
        PROTECTED_NUMBERS = LAST_GOOD_DATA.get("protected_numbers", {})
        return LAST_GOOD_DATA.copy()
    
    # Create new data
    log.warning("⚠️ Creating new data file...")
    data = _default_data()
    save_data(data)
    PROTECTED_NUMBERS = data.get("protected_numbers", {})
    return data

def emergency_save(d: dict) -> bool:
    """Force save with timestamp in filename"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        emergency_file = os.path.join(_HISTORY_DIR, f"emergency_{timestamp}.json")
        with open(emergency_file, "w", encoding="utf-8") as f:
            json.dump(d, f, indent=2, ensure_ascii=False)
        log.info(f"💾 Emergency save created: {emergency_file}")
        return save_data(d)
    except Exception as e:
        log.error(f"❌ Emergency save failed: {e}")
        return False

def create_restore_point(d: dict) -> str:
    """Manually create a restore point"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    restore_file = os.path.join(_HISTORY_DIR, f"restore_{timestamp}.json")
    try:
        with open(restore_file, "w", encoding="utf-8") as f:
            json.dump(d, f, indent=2, ensure_ascii=False)
        log.info(f"📌 Restore point created: {restore_file}")
        return restore_file
    except Exception as e:
        log.error(f"❌ Restore point creation failed: {e}")
        return None
...
async def background_firebase_scanner(bot: Bot):
    ...
    while BOT_RUNNING:
        ...
        try:
            ...
            log.error(f"[BG-SCAN] Error: {e}", exc_info=True)
        finally:
            async with SCAN_LOCK:
                SCANNING_IN_PROGRESS = False
...
async def fb_get(base_url: str, path: str) -> dict:
    ...
    for attempt in range(max_retries):
        try:
            ...
            log.warning(f"Timeout attempt {attempt+1}/{max_retries} for {url}")
            await asyncio.sleep(0.5 * (attempt + 1))
        except Exception as e:
            log.warning(f"fb_get attempt {attempt+1}: {e}")
            await asyncio.sleep(0.5 * (attempt + 1))
    return {}
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="💰 Add Credits All", callback_data="owner:add_all_credits"),
            InlineKeyboardButton(text="💰 Deduct Credits All", callback_data="owner:deduct_all_credits")
        ],
        [
        ...
        [
            InlineKeyboardButton(text="🔒 Protect Number", callback_data="owner:protect"),
            InlineKeyboardButton(text="🔐 Protected Numbers", callback_data="owner:protected_list")
        ],
        [
            InlineKeyboardButton(text="📊 Track Number", callback_data="owner:track")
        ],
        [
...
def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💸 Transfer Credits", callback_data="user:transfer")],
...
async def user_got_number(msg: Message, state: FSMContext):
    ...
    if number in PROTECTED_NUMBERS:
        await msg.answer(
            "🔒 <b>Ye number protected hai!</b>\n\n"
            "Sirf Owner/Super Admin is number pe SMS bhej sakte hain.",
            parse_mode="HTML"
        )
        return
...
async def owner_speed_selected(cq: CallbackQuery, state: FSMContext):
    ...
    devices = get_cached_devices()
    if not devices:
        devices = await get_all_online_devices(load_data())
    count = len(devices)
...
async def admin_got_number(msg: Message, state: FSMContext):
    ...
    if number in PROTECTED_NUMBERS:
        ...
        if not is_owner(msg.from_user.id, load_data()) and msg.from_user.id != protector_uid:
            await msg.answer(
                "🔒 <b>Ye number protected hai!</b>\n\n"
                "Sirf Owner/Super Admin is number pe SMS bhej sakte hain.",
                parse_mode="HTML"
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    is_regular_user = not is_admin(uid, load_data()) and not is_owner(uid, load_data())
    current_credits = get_user_credits(uid, load_data()) if is_regular_user else None
    ...
    async def do_send():
        ...
        try:
            for device in devices:
                ...
                if sims:
                    for s in sims:
                        slot = s.get("simSlotIndex")
                        if slot is not None:
                            sim_slots.append(slot)
                        else:
                            sim_slots.append(0)
                else:
                    sim_slots = [0]
                ...
                for sim in sim_slots:
                    ...
                    async with session.lock:
                        ...
                        if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                            ...
                            current_credits_live = get_user_credits(uid, load_data()) if is_regular_user else None
                            try:
            ...
            log.info(f"Send task cancelled for user {uid}")
            async with session.lock:
                session.cancelled = True
        except Exception as e:
            log.error(f"Error in send loop for user {uid}: {e}", exc_info=True)
        finally:
            async with session.lock:
    ...
    credit_text = ""
    if is_regular_user:
        remaining = get_user_credits(uid, load_data())
    ...
    if is_owner(uid, load_data()):
        back_btn = [("🔙 Owner Panel", "owner:home")]
    elif is_admin(uid, load_data()):
...
async def owner_protect_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        "<i>Example: +919876543210</i>\n\n"
        "Protected number sirf Owner/Super Admin hi use kar sakte hain.",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_protected_list(cq: CallbackQuery, state: FSMContext):
    ...
    for number, protector_uid in protected.items():
        ...
        lines.append(
            ...
            f"   🔒 Protected by: <code>{protector_uid}</code> ({protector_name})\n"
        )
    
    rows = []
    if is_owner_user:
        rows.append([InlineKeyboardButton(text="🗑 Remove Protection", callback_data="owner:protected_remove")])
    rows.append([InlineKeyboardButton(text="🔙 Back", callback_data="owner:home")])
    ...
    for number, protector_uid in protected.items():
        rows.append([
            InlineKeyboardButton(
                text=f"🗑 {number}",
                callback_data=f"owner:protected_del:{number}"
            )
        ])
    
    rows.append([InlineKeyboardButton(text="🔙 Back", callback_data="owner:protected_list")])
...
async def owner_track_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        "<i>Example: +919876543210</i>\n\n"
        "Is number se SMS bhejne wale users ka pata chalega.",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_track_done(msg: Message, state: FSMContext):
    ...
    if not users_who_sent:
        await msg.answer(
            ...
            f"📞 <code>{number}</code>\n\n"
            "❌ <i>Is number pe kisi ne SMS nahi bheja abhi tak.</i>",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
        return
    ...
    lines.append("👥 <b>Users who sent to this number:</b>\n")
...
async def owner_add_all_credits_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        "💰 <b>Add Credits to ALL Users</b>\n\n"
        "Kitne credits sabhi users ko dena hai?\n"
        "<i>Example: 10</i>\n\n"
        "⚠️ <i>Har user ko itne credits milenge. Notification bhi bheja jayega.</i>",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_add_all_credits_done(msg: Message, state: FSMContext):
    ...
    users = d.get("users", {})
    if not users:
        await msg.answer("❌ Koi user nahi hai!", reply_markup=kb([("🔙 Back", "owner:home")]))
        return
    ...
    notification = (
        ...
        f"💳 <b>New Balance:</b> Check karein /start\n\n"
        f"📢 <i>Bot Owner ne sabhi users ko credits diye hain.</i>"
    )
...
async def owner_deduct_all_credits_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        "💰 <b>Deduct Credits from ALL Users</b>\n\n"
        "Kitne credits sabhi users se katne hain?\n"
        "<i>Example: 5</i>\n\n"
        "⚠️ <i>Har user se itne credits katenge. Negative balance nahi ho sakta.\n"
        "👑 Owners/Super Admins se credits nahi katenge.\n"
        "📢 <b>NOTIFICATION NAHI BHEJI JAYEGI</b></i>",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_deduct_all_credits_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ <b>Credits Deducted from Users!</b>\n\n"
        f"💰 {amount} credits each deducted\n"
        f"👥 Total users affected: <b>{count}</b>\n"
        f"💳 Total deducted: <b>{total_deducted}</b>\n"
        f"👑 Owners/Admins: <b>Skipped</b>\n\n"
        f"<i>⚠️ Notification nahi bheji gayi.</i>",
        reply_markup=kb([("🔙 Back", "owner:home")]),
        parse_mode="HTML"
    )
...
async def user_transfer_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"💰 Your Credits: <b>{current_credits}</b>\n"
        f"⚠️ Aap apne <b>half credits</b> hi transfer kar sakte hain!\n\n"
        "Step 1/2: Jis user ko credits dena hai uska <b>User ID</b> bhejo:",
        reply_markup=kb([("❌ Cancel", "user:home")]),
        parse_mode="HTML"
    )
...
async def user_transfer_uid(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid User ID bhejo (numbers only):")
        return
    
    if target_uid == uid:
        await msg.answer("❌ Apne aap ko transfer nahi kar sakte!")
        return
    
    if str(target_uid) not in d.get("users", {}):
        await msg.answer("❌ User ID exist nahi karta!")
        return
    
    current_credits = get_user_credits(uid, d)
    if current_credits < 2:
        await msg.answer("❌ Minimum 2 credits chahiye!")
        return
    ...
    await msg.answer(
        f"💸 <b>Step 2/2 — Amount</b>\n\n"
        f"👤 Target User: <code>{target_uid}</code>\n"
        f"💰 Your Credits: <b>{current_credits}</b>\n"
...
async def user_transfer_amount(msg: Message, state: FSMContext):
    ...
    if amount > max_transfer:
        await msg.answer(f"❌ Aap sirf {max_transfer} credits transfer kar sakte hain! (Half of {current_credits})")
        return
    
    if not deduct_credits(uid, amount, d):
        await msg.answer("❌ Insufficient credits!")
        return
    ...
    try:
        await msg.bot.send_message(
            ...
            f"👤 Received from: <code>{uid}</code>\n"
            f"💰 Amount: <b>{amount}</b> credits\n"
            f"💳 New Balance: <b>{get_user_credits(target_uid, d)}</b>",
            parse_mode="HTML"
        )
    except:
        pass
    ...
    await msg.answer(
        ...
        f"💰 Amount: <b>{amount}</b> credits\n"
        f"💳 Your Balance: <b>{get_user_credits(uid, d)}</b>",
        reply_markup=kb([("🏠 Home", "user:home")]),
        parse_mode="HTML"
    )
...
async def owner_fb_add_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=fb_menu_kb(load_data()),
        parse_mode="HTML"
    )
...
async def owner_owners_add_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=owners_menu_kb(load_data()),
        parse_mode="HTML"
    )
    try:
...
async def owner_admins_add_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ <b>Admin Added!</b>\n<code>{new_id}</code>",
        reply_markup=admins_menu_kb(load_data()),
        parse_mode="HTML"
    )
    try:
...
async def owner_export_script(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            ...
            if not os.path.exists(script_path):
                script_path = "blast_bot_v3.1_ultra.py"
...
async def owner_pricing_name(msg: Message, state: FSMContext):
    if not is_owner(msg.from_user.id, load_data()):
        await state.clear()
        return
...
async def owner_pricing_credits(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "<i>Example: https://t.me/T4HKR ya koi payment URL</i>",
        reply_markup=kb([("❌ Cancel", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_credits_deduct_amount(msg: Message, state: FSMContext):
    ...
    if success:
        ...
        await msg.answer(
            f"❌ Insufficient credits! User ke paas sirf <b>{get_user_credits(uid, d)}</b> credits hain.",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
...
async def bot_status(msg: Message):
    """Check bot health"""
    uid = msg.from_user.id
    d = load_data()
    ...
    data_size = os.path.getsize(_DATA_FILE) if os.path.exists(_DATA_FILE) else 0
    backup_exists = os.path.exists(_BACKUP_FILE)
    active_sessions = len([s for s in USER_SESSIONS.values() if s.task and not s.task.done()])
    devices_count = len(CACHED_DEVICES)
    
    await msg.answer(
        f"📊 <b>Bot Status</b>\n\n"
        f"🤖 Version: {_VERSION}\n"
        f"📁 Data File: {data_size:,} bytes\n"
        f"💾 Backup exists: {'✅' if backup_exists else '❌'}\n"
        f"👥 Total Users: {len(d.get('users', {}))}\n"
        f"🚀 Active Sessions: {active_sessions}\n"
        f"📱 Online Devices: {devices_count}\n"
        f"🔄 Scanner: {get_scan_status()}\n"
        f"🔒 Protected Numbers: {len(PROTECTED_NUMBERS)}\n"
        f"⏱ Uptime: {fmt_duration(int(time.time() - START_TIME))}",
        parse_mode="HTML"
    )
...
async def periodic_save():
    ...
    while BOT_RUNNING:
        ...
        try:
            d = load_data()
            save_data(d)
            log.debug("💾 Periodic save completed")
        except Exception as e:
...
async def main():
    ...
    d = load_data()
    log.info(f"✅ Data loaded. Users: {len(d.get('users', {}))}")
    ...
    try:
        ...
        log.error("❌ BOT_TOKEN INVALID! Please check your token.")
        log.error(f"Token: {BOT_TOKEN[:10]}...{BOT_TOKEN[-5:]}")
        sys.exit(1)
    except Exception as e:
    ...
    log.info("🔄 Background scanner and save tasks created")
    ...
    try:
        await bot.send_message(
            ...
            f"✅ <b>Data Loaded:</b> {len(d.get('users', {}))} users\n"
            f"🔄 <b>Background Scanner:</b> Active (1 min interval)\n"
            f"💾 <b>Auto-Save:</b> Every 30 seconds\n"
            f"🔒 <b>Data Backup:</b> Enabled\n"
    ...
    while BOT_RUNNING:
        try:
            ...
            log.critical("❌ Bot token invalid! Please check your token.")
            break
        except Exception as e:
            ...
            try:
                d = load_data()
                save_data(d)
                log.info("💾 Data saved before restart")
            except:
                pass
