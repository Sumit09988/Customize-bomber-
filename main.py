# ARYAN_PATCHED_V19 - ULTRA OPTIMIZED
"""
╔══════════════════════════════════════════════════════════════╗
...
║  2. Add Credits to All Users                                ║
║  3. Number Tracker with Username/UserID (FIXED)             ║
║  4. Credit Transfer (Half of total) for Normal Users       ║
║  5. Protected Numbers List with Protector Details          ║
║  6. Deduct Credits from All Users (No Notification)        ║
...
class S(StatesGroup):
    ...
    deduct_all_credits_amount = State()  # New
...
def get_scan_status() -> str:
    """Get formatted scan status string for display in panels"""
...
async def owner_track_done(msg: Message, state: FSMContext):
    ...
    # Check in root sms_history (correct location - FIXED)
    ...
    for uid_str, history_list in all_history.items():
        for entry in history_list:
            if entry.get("number") == number:
                ...
                break  # One entry per user is enough
...
# ========== DEDUCT CREDITS FROM ALL USERS (NO NOTIFICATION) ==========
...
async def owner_deduct_all_credits_done(msg: Message, state: FSMContext):
    ...
    for uid_str, udata in users.items():
        ...
        if current >= amount:
            ...
            # If less credits, deduct whatever they have (make it 0)
            if current > 0:
                udata["credits"] = 0
                count += 1
...
async def main():
    ...
    try:
        await bot.send_message(
            ...
            f"💰 <b>Deduct Credits All:</b> ENABLED (No Notifications)",
            parse_mode="HTML"
        )
    except Exception as e:
