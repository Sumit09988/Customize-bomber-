...
class SendingEngine:
    ...
    async def send_sms(self, user_id, number, message, count, speed):
        ...
        devices = db.get_cache_sync().get("devices", [])
        if not devices:
...
def get_main_keyboard_sync(user_id):
    user_id = str(user_id)
    data = db.get_data_sync()
    is_owner = user_id in data.get("owners", [])
    is_admin = user_id in data.get("admins", []) or is_owner
    ...
    if is_owner:
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔵 Enable Free Mode" if not data.get("free_mode", False) else "🔴 Disable Free Mode", 
...
async def get_main_keyboard(user_id):
    return get_main_keyboard_sync(user_id)
...
async def show_settings(message: Message, user_id: str):
    ...
    await message.reply(
        ...
        f"📱 Online Devices: {len(db.get_cache_sync().get('devices', []))}\n"
        f"🛡 Protected Numbers: {len(data.get('protected_numbers', []))}\n"
